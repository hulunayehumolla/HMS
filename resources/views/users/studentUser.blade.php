...
<div class="card">
<div class="card-header bg-secondary text-white">Manage Student Users</div>
<div class="card-body overflow-auto">
      <form method="GET" action="{{ route('student.user.getlist') }}">
        <div class="row col-sm-12">
            <div class="col-sm-3">
                <label class="form-label">College</label>
              <select class="form-select form-select-sm college" name="studentCollege">
                <option value="" disabled {{ request('studentCollege') ? '' : 'selected' }}>Select College</option>
                @foreach($colleges as $college)
                    <option value="{{ $college->coll_dir_Id }}"
                        {{ (string) $college->coll_dir_Id === request('studentCollege') ? 'selected' : '' }}>
                        {{ $college->coll_dir_Name }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="col-sm-3">
                ...
                <select class="form-select form-select-sm department" name="studentDepartment">
                    <option value="">Select Department</option>
                </select>
            </div>

            <div class="col-sm-3 mt-4 py-2 ">
    ...
    <table class="table table-striped table-bordered table-sm" id="userTable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $user->username }}</td>
                <td>{{ $user->stud_Fname }} {{ $user->stud_Mname }}. {{ substr($user->stud_Lname, 0, 1) ?? 'N/A' }}</td>
    ...
    @else (empty(request('studentCollege')) || empty(request('studentDepartment')))
            <div class="text-info mt-3">
                <p>Please select both <code>College</code> and <code>Department</code> to view students.</p>
            </div>
...
//to populate department on userCollege chnages
    ...
    $(document).on('change', '.college', function () {
        ...
        var selectedDepartment = '{{ request("studentDepartment") }}'; // Get previously selected department
