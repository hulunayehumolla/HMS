...
<section id="rooms" class="py-24 bg-primary-bg px-4">
    <div class="max-w-7xl mx-auto">
        <div class="text-center mb-16">
            <h2 class="text-4xl font-extrabold text-zare-blue">Rooms & Suites</h2>
            <p class="mt-4 text-lg text-primary-text/80">Premium luxury and comfort.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
            @foreach (['normal','vip','luxury'] as $class)
                <div class="flex flex-col">
                    <h3 class="text-2xl font-bold text-zare-blue mb-6 text-center border-b pb-2 uppercase tracking-widest">
                        {{ $class }}
                    </h3>

                    @if(isset($roomsGrouped[$class]) && count($roomsGrouped[$class]) > 0)
                        <div class="swiper roomSwiper-{{ $class }} w-full pb-10">
                            <div class="swiper-wrapper">
                                @foreach($roomsGrouped[$class] as $index => $room)
                                    @php 
                                        $photo = $room->room_photos[0] ?? null;
                                        $fullImageUrl = $photo ? asset('storage/' . $photo) : asset('hotel/rooms/default.jpg');
                                    @endphp
                                    <div class="swiper-slide">
                                        <div class="rounded-2xl overflow-hidden shadow-lg bg-secondary-bg border border-gray-100 flex flex-col h-full">
                                            
                                            <div class="relative h-56 overflow-hidden group cursor-pointer" 
                                                 onclick="openGallery('{{ $class }}', {{ $index }})">
                                                
                                                <img src="{{ $fullImageUrl }}" 
                                                     class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                                                
                                                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                    <span class="text-white border border-white px-4 py-2 rounded-full text-sm">View Full Image</span>
                                                </div>

                                                <div class="absolute top-3 right-3 bg-zare-blue text-white px-3 py-1 rounded-full text-xs font-bold">
                                                    {{ number_format($room->room_price) }} ETB
                                                </div>
                                            </div>
                                            
                                            <div class="p-5 text-center flex-grow">
                                                <h4 class="text-xl font-bold text-zare-blue">Room {{ $room->room_number }}</h4>
                                                <p class="text-sm text-primary-text/70 mb-4">{{ ucfirst($room->room_type) }}</p>
                                                <button class="w-full py-2.5 bg-zare-blue text-white rounded-xl hover:bg-blue-800 transition">
                                                    Book Now
                                                </button>
                                            </div>
                                        </div>
                    ...
                    @else
                        <div class="h-48 flex items-center justify-center bg-gray-50 rounded-xl border border-dashed text-gray-400">
                            No rooms available
                        </div>
                    @endif
                </div>
...
<div id="galleryModal" class="fixed inset-0 z-[999] hidden items-center justify-center bg-black/95 backdrop-blur-md transition-all duration-300">
    
    <button onclick="closeGallery()" class="absolute top-5 right-5 text-white text-5xl font-light z-[1001] hover:text-gray-400 p-4">&times;</button>
    
    <button onclick="prevImage()" class="absolute left-4 top-1/2 -translate-y-1/2 text-white p-3 z-[1001] hover:bg-white/20 rounded-full transition">
    ...
    <div class="relative max-w-6xl w-full px-4 flex flex-col items-center" onclick="event.stopPropagation()">
        <img id="modalImg" src="" 
             class="max-h-[80vh] max-w-full rounded-lg shadow-2xl object-cover transition-all duration-300"
             style="display: block; border: 2px solid rgba(255,255,255,0.1);">
        
        <div class="mt-6 text-center text-white">
            <h3 id="modalTitle" class="text-2xl font-bold"></h3>
            <p id="modalDesc" class="text-gray-400"></p>
            <span id="modalCounter" class="inline-block mt-4 text-[10px] tracking-widest uppercase opacity-50"></span>
        </div>
    </div>

    <button onclick="nextImage()" class="absolute right-4 top-1/2 -translate-y-1/2 text-white p-3 z-[1001] hover:bg-white/20 rounded-full transition">
