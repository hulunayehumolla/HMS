...
class PharmacistController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $pharmacists = Pharmacist::with('staff')->get();
        $query = Staff::query();
        ...
        $stats = [
    'total' => Pharmacist::count(),
    'active' => Pharmacist::whereHas('staff', function ($q) { $q->where('status', 'Active'); })->count(),
    'inactive' => Pharmacist::whereHas('staff', function ($q) { $q->where('status', 'Inactive');})->count(),
              ];

        return view('pharmacist.index', compact('pharmacists','staffs', 'departments', 'stats'));
    }
/*
    // Show Create Form
    ...
    public function store(Request $request)
{
    ...
    'pharmacist_id' => 'exists:staff,staff_id',
    'qualification' => 'nullable|string|max:255',
    'license_number' => 'required|string|max:255',
            ]);
       $pharmacist= Pharmacist::create([
          
        'pharmacist_id'=>$data['staff_id'],
